...
void display_temperature_task(void *_args) {
    ...
    while (1) {
        ...
        // Display temp
        snprintf(str, sizeof(str), "%.0f", (temperature*1.8) + 32);
        ...
        ssd1306_draw_string(&display, display_buffer, font_builtin_fonts[DEFAULT_FONT3], 36, 0, "\xB0", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
        ssd1306_draw_string(&display, display_buffer, font_builtin_fonts[DEFAULT_FONT3], 100, 0, "\xB0", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
        // Display humidity
		...
		if (state_value == 1){
			ssd1306_draw_string(&display, display_buffer, font_builtin_fonts[DEFAULT_FONT3], 112, 0, "\x60", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
        } else if (state_value == 2){
			ssd1306_draw_string(&display, display_buffer, font_builtin_fonts[DEFAULT_FONT3], 112, 0, ".", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
        } else {
            // doing nothing
        }
